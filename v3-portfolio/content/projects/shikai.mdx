...
live: "https://play.google.com/store/apps/details?id=com.atharvdange618.Shikai"
...
- **Deep Repo Navigation**: Blame and per-file commit history, a compare view between any two refs, a releases tab with changelogs and downloadable assets, and a Discussions viewer.
- **Richer PR/Issue Threads**: Review comments show their diff hunk inline, timelines merge label/state/reference events into the comment stream in order, and reactions show up under bodies and comments.
- **GitHub URL Deep Links**: Opening or sharing a github.com link (repo, issue, PR, commit, compare, release, file, folder) routes straight into the matching in-app screen instead of a browser.
- **Crash and Performance Monitoring**: Anonymous crash reports and sampled performance data sent to Sentry, with GitHub tokens stripped from every report before it leaves the device. Off in development.
...
### 6. The Play Store Review Wall

Google Play rejected a release over the OAuth sign-in flow. A reviewer's sandboxed session can't complete the browser redirect and device-verification/2FA prompts that a real user's session sails through, so the sign-in just stalled during review.

The fix was a second, self-contained entry point: a "Sign in with a token" option that accepts a classic or fine-grained PAT with `repo` and `read:user` scopes, validates it directly against the GitHub API, and starts the session without touching a browser at all. It's not the path I expect real users to take, it exists so a reviewer (or anyone without OAuth access) has a way in that never depends on GitHub's own auth UI. Both paths converge on the same `finalizeSession()` call, so the rest of the app has no idea which one ran.
...
The codebase is at [github.com/atharvdange618/Shikai](https://github.com/atharvdange618/Shikai). Shikai is live on the [Google Play Store](https://play.google.com/store/apps/details?id=com.atharvdange618.Shikai), with release APKs also available on the [GitHub releases page](https://github.com/atharvdange618/Shikai/releases).
