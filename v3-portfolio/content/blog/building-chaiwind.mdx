...
<Callout type="warning">
  **`cssText` is destructive.** Setting `el.style.cssText = "padding: 8px"` replaces ALL inline styles on that element. If any other code set `color: red` inline, it's gone. Use `el.style.setProperty(property, value)` to set one property without touching the rest. This is the correct choice for any runtime style injection system.
</Callout>
...
<Callout type="info">
  **RAF batching prevents redundant work.** Without batching, inserting 50 nodes at once fires the MutationObserver callback 50 times, calling `processElement()` 50 times separately. With RAF batching, all 50 nodes are collected into a Set (automatic deduplication), and a single `requestAnimationFrame` processes them all in one pass. This is the difference between O(n) callbacks and O(1) callback per frame.
</Callout>
