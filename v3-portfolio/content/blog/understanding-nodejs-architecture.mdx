...
Three layers. Let's go through each one.
...
<Callout type="warning">
  **Thread pool saturation is a silent killer.** The default pool size of 4 is a common source of latency spikes that are hard to diagnose. Under moderate load, requests queue up silently. Your response times spike without any obvious error in your logs. Monitor `UV_THREADPOOL_SIZE` and profile under realistic concurrency.
</Callout>
...
<Callout type="warning">
  **`process.nextTick` recursion starves I/O.** If a `nextTick` callback schedules another `nextTick`, the event loop will keep processing them before moving on. No I/O callbacks, no timers, no setImmediate - nothing else runs. Use `setImmediate` for most cases. It fires after I/O callbacks, so it can't starve the system.
</Callout>
...
<Callout type="error">
  **Blocking the event loop is the #1 Node.js performance mistake.** A single synchronous operation that takes 200ms blocks every other pending callback for at least 200ms. On a server handling thousands of requests, one blocking call can cascade into widespread latency. If you can't avoid CPU-intensive work, offload it to a worker thread.
</Callout>
...
<Callout type="info">
  **"Node.js is single-threaded" is a dangerous oversimplification.** Your JavaScript runs on one thread. But libuv has a thread pool (default 4 workers), the OS handles network I/O via kernel threads, and `worker_threads` gives you actual multi-threading. The JavaScript execution model is single-threaded. The runtime is not.
</Callout>
...
<Callout type="warning">
  **`setTimeout(fn, 0)` does not mean "run immediately after current code."** It means "schedule for the timers phase of the next event loop iteration." Before it runs: synchronous code finishes, `process.nextTick` queue drains, Promise microtask queue drains. The delay is a minimum, not a guarantee - if the event loop is busy, your timer fires late.
</Callout>
