---
title: "Understanding Node.js Architecture: How V8, libuv, and the Event Loop Fit Together"
...
summary: "Node.js is a carefully assembled runtime, not just 'JavaScript on the server.' V8 compiles JS to machine code, libuv provides the event loop and thread pool for async I/O, and the C++ bindings connect them. Understanding this architecture explains why Node.js handles concurrency with a single thread, when worker threads are actually needed, and how non-blocking I/O really works under the hood."
---
...
V8 doesn't compile everything upfront. It starts by interpreting your code (Ignition), which is fast to start but slower to execute. As it runs, it identifies "hot" functions, ones called frequently, and passes them to TurboFan, the JIT (Just-In-Time) compiler, which produces optimized machine code for them.
...
V8 makes assumptions when optimizing code. If you write a function that always receives a number, TurboFan will compile a highly optimized version for numbers. But if you then pass a string to that same function, V8 has to "deoptimize" back to the interpreter. This is called a **deoptimization bail-out**.
...
The practical takeaway: keep your functions type-consistent. Mixing types in hot paths hurts performance. TypeScript nudges you toward consistent typing, which is one of its biggest hidden wins at the application level.
...
Most people misunderstand this: the event loop is not part of V8. V8 handles JavaScript execution. libuv handles everything else: file system operations, network I/O, DNS lookups, child processes, and the event loop that orchestrates all of it.
...
Node.js is not entirely single-threaded.
...
<Callout type="warning">
  **Thread pool saturation is a silent killer.** The default pool size of 4 is a
  common source of latency spikes that are hard to diagnose. Under moderate
  load, requests queue up silently. Your response times spike without any
  obvious error in your logs. Monitor `UV_THREADPOOL_SIZE` and profile under
...
The event loop is the heart of Node.js. It's what lets a single thread handle thousands of concurrent operations. But "event loop" is often explained in hand-wavy terms. Here is the precise version.
...
Between each phase transition (and also between callbacks within a phase in newer versions of Node.js), the event loop processes two special queues:
...
<Callout type="warning">
  **`process.nextTick` recursion starves I/O.** If a `nextTick` callback
  schedules another `nextTick`, the event loop will keep processing them before
  moving on. No I/O callbacks, no timers, no setImmediate - nothing else runs.
  Use `setImmediate` for most cases. It fires after I/O callbacks, so it can't
  starve the system.
</Callout>
...
<Callout type="error">
  **Blocking the event loop is the #1 Node.js performance mistake.** A single
  synchronous operation that takes 200ms blocks every other pending callback for
  at least 200ms. On a server handling thousands of requests, one blocking call
  can cascade into widespread latency. If you can't avoid CPU-intensive work,
...
From `node server.js` to handling your first request:
...
<Callout type="info">
  **"Node.js is single-threaded" is a dangerous oversimplification.** Your
  JavaScript runs on one thread. But libuv has a thread pool (default 4
  workers), the OS handles network I/O via kernel threads, and `worker_threads`
  gives you actual multi-threading. The JavaScript execution model is
  single-threaded. The runtime is not.
</Callout>
...
<Callout type="warning">
  **`setTimeout(fn, 0)` does not mean "run immediately after current code."** It
  means "schedule for the timers phase of the next event loop iteration." Before
  it runs: synchronous code finishes, `process.nextTick` queue drains, Promise
  microtask queue drains. The delay is a minimum, not a guarantee - if the event
  loop is busy, your timer fires late.
</Callout>
...
**V8 parses, interprets via Ignition, and JIT-compiles hot code via TurboFan.** It manages your heap and runs garbage collection. Type-consistent functions perform better because V8 optimizes based on type assumptions.

**libuv owns the event loop and the thread pool (default 4 workers) for operations the OS can't do asynchronously.** Network I/O bypasses the thread pool entirely, going through OS-native async (epoll, kqueue, IOCP).

**Node.js bindings are the C++ glue between JavaScript and libuv.** `fs.readFile` is JavaScript calling C++ calling libuv. Native addons extend this, and N-API keeps them stable across versions.

**The event loop has distinct phases.** Microtasks (`nextTick`, then Promises) drain between every phase transition. The poll phase is where the loop waits for I/O.

**Node.js is a smart assembly of well-designed components solving a specific problem: handling I/O-bound concurrency on a single JavaScript thread.** Stay in that lane and it's exceptional. Throw long-running CPU work at it without worker threads and you're fighting its design.
