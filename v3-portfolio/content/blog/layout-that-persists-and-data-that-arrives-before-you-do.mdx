...
One line changed: `children`. The root layout wraps everything. When you navigate to `/about`, the router does not just find the About component. It finds the root layout AND the About component. Two matches, not one.
...
Here is the thing that trips people up. The route definitions are a tree, but the matched result is a flat array. Why? Because React renders top to bottom, linearly. The tree structure is just for configuration. Once matching is done, you flatten it into an ordered list: root first, then the matched child. That flat array is what `<Outlet />` walks through.
...
There is a key distinction here. `__root` always contributes to the match chain. It is the outermost shell - it must always render. But other layout routes (`_auth`, `_dashboard`, etc.) only contribute when a descendant actually matches. If no child matches, the layout is skipped via `continue`, letting sibling routes try.
...
That is it. No magic, no diffing, no special React behavior. It is a context consumer that does an array lookup.
...
Here is the thing that makes it click. The `OutletContext` holds a single number: the current depth into `state.matches`. `RouterProvider` renders `matches[0]` directly and sets `OutletContext` to `1`. Each `<Outlet />` reads its index, renders that match, and increments for the next level.
...
Here is why this matters. Navigate to `/about` with this route tree:
...
Here is what happens when you navigate to `/settings/profile`:
...
Here is where the Outlet pattern from Slice 3 becomes essential for more than just layouts.
...
Here is the thing that makes this clever. `OutletContext` holds the index of the NEXT match to render. When `UserPage` is mounted by an `<Outlet />` at depth `1`, `OutletContext` inside `UserPage` is `2`, because Outlet set it to `matchIndex + 1` before rendering the component.
...
Here is the problem with async loaders. You click Atharv, a 600ms fetch starts. Before it resolves, you click John, another 600ms fetch starts. If John's fetch resolves first, then Atharv's resolves, you would end up showing Atharv's data even though you are on John's URL.
...
Here is the problem with immediately showing a loading state on every navigation. If a loader resolves in 50ms, the user never sees the loading state, but React still unmounts the old page, shows a spinner for one frame, then mounts the new page. That flash is jarring.
...
Here is how `commitNavigation` calculates and enforces that minimum display duration before committing the state:
...
The key insight: the loading state is no longer "start loading immediately." It is "show a loading indicator if the navigation is slow." Fast navigations feel instant. Slow ones get a spinner.
...
**The tree solves ownership, not URLs.** Route nesting is about which layout wraps which pages, not about URL structure. `/settings/profile` is still a full absolute path. Nesting it under `/settings` just says "render inside `SettingsLayout`."
...
**React's reconciler does the layout stability.** Same component at the same position in the tree means no remount. The router does not do anything special to keep the nav alive - it just ensures `matches[0]` is always `RootLayout`, and React handles the rest.
