...
tags:
  ...
  - "api-design"
featured: true
description: "A deliberately flawed JWT auth system, designed as a teaching exercise to surface the security holes, race conditions, and missing edge cases that survive a first-pass auth design."
...
Here's the setup: I had an assignment to deepen my understanding of auth systems. Instead of building a correct one and explaining it, I took a different approach. I designed a complete JWT auth system with real-looking flows, real endpoints, and real security decisions. Then I had it reviewed. The goal was to find what kind of issues a well-intentioned but imperfect auth design would produce, and to write those issues up so that when you build your own system, you know what to look for.

Think of your auth system like a bank vault. You can have the best door in the world: reinforced steel, biometric lock, time delay mechanism. But if the ventilation shaft is wide enough to crawl through, none of that matters. The attacks you didn't account for are the ones that matter most.
...
<Callout type="error">
  **bcrypt truncation is silent and dangerous.** A 200-character password and a
  73-character password sharing the first 72 bytes produce the same hash. The
  user never knows their password got cut off. Always validate password length
  in bytes (not characters) before hashing.
</Callout>
...
72 is exactly the bcrypt truncation limit. Note the check uses `Buffer.byteLength` (bytes), not `.length` (characters). A single Unicode character can be 2 to 4 bytes, so a "short" password with emoji or non-ASCII characters can exceed the limit sooner than expected. The error message should reflect bytes, not characters.
...
<Callout type="warning">
  **Timing attacks are real.** An attacker can measure response times to
  determine whether an email exists in your database. Fast response = email not
  registered. Slow response (bcrypt compare) = email registered, wrong password.
  Always run bcrypt against a dummy hash for non-existent users to equalize
  timing.
</Callout>
...
<Callout type="error">
  **This is the most critical issue in the entire system.** A normal SPA fires
  multiple API calls simultaneously. When the access token expires, all of them
  try to refresh. Without client-side deduplication, the second request triggers
  a false reuse attack and logs the user out. The fix lives in the Axios
  interceptor, not the backend. Auth is a distributed system - design it like
  one.
</Callout>
...
This is documented behavior in most JWT systems. Know this window exists and communicate it - don't assume tokenVersion in the refresh JWT solves it.
...
First, atomicity matters more than you think. Any time you check a condition and then modify state based on it, those two operations need to be atomic. Separate check plus write is a race condition waiting to happen. Use DB transactions, conditional updates with WHERE clauses, and Redis NX operations.

<Callout type="info">
  **The atomicity rule:** Any time you check a condition and then modify state
  based on it, those two operations need to be atomic. Separate check + write is
  a race condition waiting to happen. Use DB transactions, conditional updates
  with WHERE clauses, and Redis NX operations.
</Callout>
